...
**상태: 진행 중.** 8단계 중 0·1단계 완료, 2단계는 측정 완료·검증 대기 -- 백엔드 조사,
3번째 백엔드 확정, 환경 3종 구축 및 GPU 동작 검증, 하네스 뼈대, **PyTorch 레퍼런스 측정**.
...
reduction 순서, attention 백엔드와 설정 차이가 함께 들어가므로 내부 eager↔SDPA 비교점과
...
| D3 | logits를 **버전이 붙은 경로**로 디스크에 캐시한다 | 백엔드를 순차 실행하고 나중에 파일로 비교한다. 버전 가드가 없으면 옛 정의로 만든 결과가 새 결과와 조용히 섞인다. 사이드카에 토큰 ID를 저장해 동일 입력을 증명하되, 공개 token IDs는 공개·합성 프롬프트에만 허용한다 |
