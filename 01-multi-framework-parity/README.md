# 01. 멀티 추론 프레임워크 수치 정합성(Parity) 검증 하네스

**상태: 진행 중.** 8단계 중 0·1단계 완료, 2단계는 측정 완료·검증 대기 -- 추론 프레임워크 조사,
3번째 추론 프레임워크 확정, 환경 3종 구축 및 GPU 동작 검증, 하네스 뼈대, **PyTorch 레퍼런스 측정**.
추론 프레임워크 간 정합성 측정값은 아직 없다. 첫 비교는 3단계에서 나온다.

2단계 결과는 [`results/reference/README.md`](results/reference/README.md)에
있다 -- 텍스트 전용 VRAM 실측, 파라미터 분포, 어텐션 배치.

3단계는 [`03-vllm-parity-plan.md`](03-vllm-parity-plan.md)를 따른다. vLLM v0.26.0
raw-logits 스파이크, 레퍼런스 검증, 계약·공개 meta 경계 동기화, 환경·코드
manifest 고정, 본편 2K, 최종 동기화를 차례로 통과하기 전까지 본 측정은 No-Go다.

## 답하려는 질문

추론 프레임워크 두 개에 같은 모델을 올렸는데 logits가 다르게 나온다.
**이 차이는 받아들일 수 있는 수치 오차인가, 포팅 버그인가?**

눈으로 판단하는 방식은 확장되지 않는다. 이 프로젝트는 그 질문에 **숫자와 명시된
임계값으로** 답하는 하네스를 만들고, 그 하네스로 추론 프레임워크 4종과 양자화 4계층을
비교한다.

## 접근

```
PyTorch 레퍼런스 -> 추론 프레임워크 어댑터 -> 양자화 계층 -> logit parity + 정확도 + 프로파일링
```

핵심은 **비양자화 baseline을 먼저 세우는 것**이다. 양자화를 적용하기 전에,
같은 가중치를 같은 정밀도로 올린 PyTorch와 vLLM을 먼저 비교한다. 이 값에는 커널 선택,
reduction 순서, attention 구현와 설정 차이가 함께 들어가므로 내부 eager↔SDPA 비교점과
환경 manifest를 나란히 기록한다. 이후 양자화 결과는 이 baseline 위에서 추가로 벌어진
몫으로 해석한다.
...
| OpenVINO | 지원, **3번째 추론 프레임워크로 채택** | OpenVINO 조직이 E2B IR 배포. PLE / language model / vision이 파일 단위로 분리돼 있다 |
...
**3번째 추론 프레임워크 선택은 조사 중에 뒤집혔다.** 프레임워크 독립성 때문에 ONNX Runtime을
...
| | 결정 | 근거 |
|---|---|---|
| D1 | `logits()`는 문자열이 아니라 **토큰 ID**를 받는다 | 추론 프레임워크마다 자기 토크나이저로 자르게 두면 토크나이즈 차이가 측정값에 섞인다. 한 번만 토크나이즈해서 같은 정수열을 모든 추론 프레임워크에 먹인다 |
| D2 | 어댑터는 **컨텍스트 매니저**다 | 모델 10.25GB에 VRAM 11.9GB. 두 추론 프레임워크가 동시에 상주할 수 없으므로 해제는 필수이고, `with` 블록은 `unload()` 호출과 달리 **잊을 수가 없다** |
| D3 | logits를 **버전이 붙은 경로**로 디스크에 캐시한다 | 추론 프레임워크를 순차 실행하고 나중에 파일로 비교한다. 버전 가드가 없으면 옛 정의로 만든 결과가 새 결과와 조용히 섞인다. 사이드카에 토큰 ID를 저장해 동일 입력을 증명하되, 공개 token IDs는 공개·합성 프롬프트에만 허용한다 |
| D4 | 비교기는 **숫자만 내고 판정하지 않는다** | 임계값은 설정 파일에 두고 하류에서 적용한다. 판정을 비교기 밖에 두면, 결과가 나쁠 때 임계값을 넓혀 해결하는 길이 막힌다 |
| D5 | 원문 meta와 공개 meta를 **분리**한다 | vLLM에 `awq`를 요청해도 `awq_marlin`이 돌아갈 수 있으므로 원문은 비공개로 보존한다. 공개 sidecar는 버전·커널·크기·메모리 허용목록만 내보내고 인증·환경변수·절대 경로는 fail-closed로 차단한다 |
| D6 | 결정론성을 체크리스트가 아니라 **코드로** 확인한다 | `load()`가 `logits()`를 두 번 호출해 바이트를 비교한다. 자기 자신을 재현하지 못하는 추론 프레임워크는 그 자리에서 실패한다. 며칠 뒤에 커널 차이처럼 보이는 잡음으로 나타나지 않는다 |
...
| D9 | 계약이 옳았는지의 **판정선을 미리 등록**한다 (3단계 종료 시 태그) | 3번째 추론 프레임워크를 붙이는 단계는 "어댑터 하나만 추가하면 끝나는가"로 설계를 판정한다. 판정 기준을 아는 사람이 사후에 판정하면, 앞 단계에 본체를 미리 넓혀두는 것만으로 통과를 만들 수 있다. 3단계를 마칠 때 계약을 git tag로 고정하고 `harness/adapters/base.py` · `compare.py` · `cache.py` 중 하나라도 바뀌면 "본체를 고친 것"으로 센다. 판정은 그 태그 대비 `git diff`로 읽는다 |

> **D9 동결 순서:** 3단계 종료 전에 더미 어댑터를 본체 수정 없이 추가하는
> 스모크를 먼저 통과한다. 그 후에만 계약 tag를 만든다.
...
나머지 추론 프레임워크 어댑터는 3·5·6단계에서 추가되며 역시 GPU 머신이 필요하다.

## 구조

```
01-multi-framework-parity/
  harness/
    adapters/    추론 프레임워크마다 하나. 공통 인터페이스 뒤에 둔다
